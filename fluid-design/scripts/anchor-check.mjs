...
// well setup end to end: `verify-matrix.mjs --reveal` forces
...
// Resolution order matches verify-matrix.mjs/probe.mjs: the target project's
// own node_modules first (resolved from process.cwd()), then a bare
// specifier from the skill's own location. Kept duplicated rather than
// shared through scripts/lib, which is the config generator's tree.
...
async function checkOneAnchor(page, hash, headerVar, tolerance) {
  ...
  // verify-matrix.mjs's reveal step and probe.mjs).
  await page.evaluate(
    () =>
      new Promise((resolve) => {
        let last = -1

...
// platform, built by `bun build --compile` from scripts/bin-entry.mjs, which
...
//
//   node scripts/build-bin.mjs                 every target → dist/
//   node scripts/build-bin.mjs --target host   only this machine's
//   node scripts/build-bin.mjs --smoke         then run the host binary through
...
for (const [os, arch] of targets) {
  ...
  const r = spawnSync('bun', ['build', '--compile', '--minify', `--target=bun-${os}-${arch}`, join(SKILL, 'scripts/bin-entry.mjs'), '--outfile', file], { encoding: 'utf8' })
  if (r.status !== 0) {

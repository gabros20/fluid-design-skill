...
// :root plus the mobile bands (tablet, landscape) that refine it below
// engageAt; empty bands list without the mobile arm.
function rootBlock(units) {
  const lines = Object.entries(units.root).map(([k, v]) => `  ${k}: ${v};`)
  const bands = (units.bands ?? [])
    .map((b) => `@media ${b.media} {\n  :root {\n${Object.entries(b.vars).map(([k, v]) => `    ${k}: ${v};`).join('\n')}\n  }\n}\n`)
    .join('')
  return `:root {\n${lines.join('\n')}\n}\n${bands}`
}
...
${cfg.mobile.enabled ? `  /* Mobile arm: the phone frame's 24px gutter, scaled, and on tablet and
     landscape a centred column (--fluid-column) so the phone design keeps
     its line lengths instead of stretching across the screen. */
  padding-inline: calc(24 * var(--fluid));
  max-width: var(--fluid-column);
}` : `  padding-inline: 24px;
}
@media (width >= 640px) {
  .${p}-frame {

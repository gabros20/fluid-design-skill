...
**Read when:** you're deciding how a new animated section should be built — component vs. copied
pattern vs. plain library calls — or reviewing whether an existing one follows the house rules.
**Skip when:** you already know the shape you want and just need the pin/scroll mechanics (see
`scroll-scenes.md`) or a video-specific trick (see `video.md`).
**Depends on:** `fluid-scale.md` for the CSS-variable convention distances are written in;
`tokens-and-theming.md` for how motion tokens sit next to design tokens.
...
`gsap.timeline()` equivalent — build a container of time, and that's the whole job." It got built.
The site that shipped needed **one** scroll-driven scene, and that one scene needed none of the
compiler — it needed mode hysteresis, a frame-accurate media wrap, a handoff glide and a camera
(all covered in `scroll-scenes.md` and `video.md`). The timeline layer solved a problem the build
didn't have.

Generation 3 survives because every piece of it is **specific** — it names a job (an entrance, a
count, an exit-fade, a pinned scrubbed render) rather than a mechanism. That specificity is what
the extraction rule below turns into policy.
...
its landmarks — under 300 lines in the reference build's one scrubbed scene, none of which would
have been shorter with a compiler.
...
Both deleted generations violated this — the timeline layer and the debug panel each had at most
one consumer. The corollary is what keeps it honest: **when the second consumer appears, extract
what the two share — not what you imagine a third will want.** An exit-fade component gained a
...
not a component — the direct style write (§9 below, full mechanics in `scroll-scenes.md`). The three
call sites share no code: one subscribes to a value and writes `opacity`, one writes four quantised
custom properties from an rAF tick, one writes a composed transform string from two clocks at once.
A helper general enough to cover all three would be longer and harder to read than any of them.
...
- **One range wrapper per stack, never one per section.** Sections handing off to each other need a
  shared ruler; per-section rulers can't express the overlap (`scroll-scenes.md` §Pin).
- **Only 1–3 scroll-driven scenes should intersect the viewport at once.** At forty animated
  sections this triage *is* the perf budget — see `performance.md`.
- **Never demote a scroll scene to a trigger silently on mobile.** If mobile wants a different
  treatment, say so explicitly in the component — and note "different" may mean *re-framed* rather
  than removed (the camera pattern, `video.md` §Reframe).
...
```tsx
// Motion — assets/motion/react-motion/components/Stage.tsx et al.
<Stage trigger="view" className="flex flex-col gap-4">
  <StageItem variant="liftFade" className="[--lift:20px] lg:[--lift:24px]">
...
one stage it fires once, at the top edge — the baseline mark plays its entrance while it's still
most of a screen below the fold and finishes long before anyone scrolls to it, which reads as "that
element doesn't animate." Two stages, each waiting for its own arrival, costs nothing and is the
fix. The same failure appears whenever a wide row stacks into a tall column on mobile — what was
one arrival on desktop becomes two.
...
- **`amount` as a fraction is a trap on tall content.** `0.6` of an element taller than the viewport
  ...
  same content stacks tall (mobile, usually). Desktop is fine; the phone ships permanently blank.
  Set the trigger line with `margin` and leave `amount` alone.
...
| --- | --- | --- | --- |
| `REVEAL_TRIGGER` | `0px 0px -20% 0px` | every below-the-fold section | the general default — was `-35%` once, which read late on tall content |
| `PAGE_END_TRIGGER` | `0px` | the last group on the page (footer) | a negative bottom margin draws the line *inside* the viewport, so something must scroll *across* it — the terminal screenful never does, and the scroll runs out first. Measured: nine footer items never fired under the general default. |
| `AT_REST_TRIGGER` | `0px` | a group whose *resting* scroll position sits below the inset line | the second way the trap above bites, and not at the end of the page — a scroll well (`scroll-scenes.md` §Scroll well) gives a section a resting position, and content low in a section taller than the viewport never scrolls across an inset line at that rest state either |
...
is camera motion blur, not a designed filter — see `video.md`'s confound tests for how to tell them
apart). Keep the vocabulary that thin; the economy is the style, and it's also what stays under
`performance.md`'s per-frame budget.
...
.hidden { transform: translateY(var(--lift, 32px)); opacity: 0; }
...
This is the same tier-1 responsive convention `fluid-scale.md` and `tokens-and-theming.md` use:
vary the value with a plain breakpoint utility and zero JS, let the runtime resolve the `var()` at
animation start.

One variant, `growY`/`growX`, is the sanctioned exception to "never animate layout properties" —
...
- **The fade deliberately runs on a different clock than the move.** In the reference the move
  ...
  clears; the stagger still technically happens, nobody ever sees it.
...
fades the *group* on scroll (never per-element — see `scroll-scenes.md`'s note on why a 46px
wordmark fading across 46px of scroll snaps instead of fading); they compose because they touch
different things, and an item that has finished its entrance sits at opacity 1 for the group fader
to multiply over.
...
product UI can't — not a licence for a nav or a header a returning visitor sees on every page load.
...
decorative — never block interaction while it plays. For a scroll-driven stagger, convert before
...
scroll spring is deliberately over-damped (ζ ≈ 1.37) because a scroll spring that overshoots crosses
boundaries it shouldn't (see `scroll-scenes.md` §A spring never feeds a threshold).
...
stylesheet rather than scattered as utility classes, because a utility collides with the runway
utilities at equal specificity and which one wins becomes a build-order accident.
...
1. **Content is always in the DOM.** Animations change `transform`/`opacity`, never conditional
   rendering — crawlers and readers get the full text regardless of JS.
2. **A stage serialises its hidden variant into the SSR HTML**, so there's no flash of the settled
   layout. The cost is the entrance can't start until hydration — which is what a load veil covers.
3. **A `<noscript>` rule forces every stage item visible** and hides the load veil. Without it a
   visitor with JS disabled, or a script that failed to load, gets a hero and then a permanently
   ...
   no JS it has no way to ever leave 0.
...
5. **Never branch rendered structure or element type on a client-only breakpoint hook.** It reports
   ...
   safe, branching the element *type* is not (see Traps).

## 13. The GSAP escape hatch

The reference build's default is a single JS animation library (Motion, in that build) with no
second engine — but the decision was made falsifiable rather than dogmatic. Before committing to
one library for an unfamiliar codebase, the panel that made this call ran a real experiment: build
the first heavy pinned non-video scene with the default library only, then decide on evidence,
against kill criteria fixed *in advance*:
...
| Property values (`from`/`to`) inside a timeline spec | Requires reading computed styles to know both endpoints — breaks determinism across SSR, resize, remount, reverse seek. |
...
| A scroll-smoothing library (Lenis-class) | Its lerp reshapes the native velocity curve anything scroll-linked reads, and gains nothing on iOS touch. Smoothing belongs on effect *outputs* (a spring on a transform), never on the scroller. |
| A momentum-handoff spring fired at a scroll crossing | Needed velocity tracking, absorb/veto budgets and a rest-detection override to avoid feeling like a scroll lock — and still read as "the page waits for you to stop, then grabs you." See the continuous-attractor pattern in `scroll-scenes.md` instead. |
...
- A component built for its first caller is that caller with indirection — wait for the second (§2).
- `viewport.amount`/similar as a *fraction* is unsatisfiable on content taller than the viewport —
  fails silently, phone-only (§6).
- A tall stage group fires on its own top edge — split it into one stage per arrival, not one per
  layer (§5).
- A stage item given its own `initial`/`animate` (or its own GSAP trigger) severs from its stage and
  animates on its own schedule (§4).
- A fresh `rootMargin`/offset literal built per render resubscribes the observer every render — use
  a module constant.
- Easing an already scroll-driven value is double-easing (§10).
- A spring's output feeding a boolean/threshold test rings true/false/true as it settles — read raw
  progress for any machine (`scroll-scenes.md`).
- Branching element *type* (not just behaviour) on a client-only breakpoint hook remounts the
  subtree at hydration and strands already-fired items hidden forever (§12, and `scroll-scenes.md`).
- A hook called inside `.map()` only "works" while the array length never changes — extract the
  child component.
- A global reduced-motion config cannot reach a hand-written `style` write — every manual writer
  checks the media query itself (§11).
- Two Tailwind `transition-*` utilities (or two CSS `transition` shorthands) on one element both set
  `transition-property`; which one wins is stylesheet-order luck, not the order they're written in.

...
min-keyint=1, scenecut=0`) makes every seek a single-frame decode.
...
shipping. Reach for all-intra only when the component genuinely maps scroll position to `currentTime`.
...
consecutive frames across the clip's steady (non-intro) section — that average is the "motion
already present" baseline. Any candidate seam costing at or below that floor is invisible, because
it differs by less than two adjacent real frames already differ by:
...
directly, clearing any pending seek first. A wrap queued behind a stale pending seek arrives a
frame late, which reintroduces exactly the flash this coalescing exists to prevent elsewhere.

Also: Safari specifically queues seeks and falls behind if more than one is genuinely in flight —
an imperative controller layer should enforce "one seek in flight at a time" as a hard rule, with a
sub-frame epsilon below which a seek is skipped entirely rather than issued (roughly half a frame at
the asset's fps is a reasonable floor).
...
even one scrubbed scene affordable at all (`performance.md`).
...
compositing anchor, not decoration — it's one of the few standing exceptions to "no `will-change`,
no forced compositing" in `performance.md`, and it should stay inside whatever composed transform
string a camera or scrub scene writes per frame (`scroll-scenes.md` §The direct style write), not as
a separate declaration that a later refactor might drop.
...
poster swaps tier while the video keeps its original file — the two backgrounds now draw at
different scales with a hard edge between them ("double background").
...
The playhead carries across; if the resumed `play()` is refused for lack of a user gesture, catch
it and let whatever UI mirrors playback state (§11) fall back to its paused/idle rendering. This
matters less than it sounds for pure-phone traffic (nothing loads a desktop tier and then narrows
below it), but it's real on a desktop window dragged across the breakpoint and on tablet rotation —
worth checking which physical devices straddle your tier boundary before shipping one.
...
(well under a second) that's a visible hitch on every single repeat, not just the first. Re-encoding
such an asset without preserving this forced keyframe is a regression that won't show up until
someone notices the loop stutter in production.
...
is uncontroversial — gate them on an `IntersectionObserver` (§5) rather than firing at mount, so
nothing plays off-screen.

If a control mirrors play/pause state in the UI, make the **element** the single source of truth and
have the UI only mirror it — every path that can change playback (a click, a refused programmatic
...
4. Derive mode and target playhead time **absolutely** from that progress (not via the live
   edge-plus-hysteresis stepper §3 in `scroll-scenes.md` uses for in-session transitions — a cold
   resume wants an unambiguous answer, not a state machine that assumes it already knows where it
   was).
...
7. If now awake, **snap** the playhead to the derived time — no glide. A live in-session handoff
   between modes glides (`scroll-scenes.md`'s handover pattern); a cold resume snaps, because there's
   no "in between" frames worth playing on the way from a stale time to the correct one.
...
difference between a five-minute diagnosis and an unreproducible ticket.
...
- Large flat colour fields band before detail does — that's where to spend CRF budget; a
...
- ★ A non-all-intra asset makes scrubbing a slideshow — every seek decodes from the nearest keyframe (§1).
- ★ All-intra on a video that only plays forward pays a real bitrate cost for nothing (§1).
- ★ Polling `currentTime` inside rAF cannot wrap a loop frame-accurately — use `requestVideoFrameCallback` (§3).
- ★ A seek issued while one is outstanding is silently dropped, not queued (§4).
- ★ Calling `.load()` on the warm preload tier aborts an in-progress `play()` on phones (§5).
- ★ A CSS reset's `video { max-width: 100% }` silently clamps a deliberately oversized scrub video —
  `max-w-none` (§7).
...
- Probing permission and unmuting a live audio element mid-session is engine-quirk territory — prefer
  a gesture-backed `play()` inside a click handler (§11).
- Trusting a stale `currentTime`/`awake` flag after a tab wakes from sleep desyncs the render from
  scroll — always rehydrate from scroll progress on resume, never resume-in-place (§12).
- A big "master" file is not evidence of quality — probe codec, GOP and bitrate before trusting its
  size (§1, §13).

...
A framework-agnostic port of the fluid-design motion system's behaviour,
...
3. Call the init function once, after the DOM you want wired is present:
   ...
   ```ts
   import { initFluidMotion } from 'fluid-design/motion/gsap'
   // or, from source: import { initFluidMotion } from './assets/motion/gsap/src'
...
- **The reference build's `ScrubStage` camera numbers (`SHOTS`, `SUBJECT`,
  ...
  — see the reference build's `VIDEO-AND-MOTION-TRICKS.md` Recipes 3, 5 and
  6 for how those numbers get measured in the first place.

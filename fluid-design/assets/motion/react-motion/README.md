...
bundler. If your project uses Tailwind, also grab `cn` from
`assets/styles/tailwind-v4/cn.ts` (the fluid-design skill's Tailwind package)
and swap it in for `lib/cx.ts`'s bare joiner — see **Styling** below.
...
lib/
  ...
  constants.ts          `ENGAGE_QUERY` — the one breakpoint every primitive
                     agrees on. Must match `fluid.config.json`'s `engageAt`.
  cx.ts                  Dependency-free class joiner. Swap for the fluid
                     Tailwind `cn` if you have it — see Styling.
...
If your project has the fluid-design Tailwind package
(`assets/styles/tailwind-v4/`), swap `lib/cx.ts`'s import for its `cn.ts` in
any file that imports `cx` — that version additionally resolves
Tailwind-class conflicts (e.g. two `fluid-p-*` utilities on one element)
correctly, which the bare joiner here does not attempt.
...
- **The fluid scale** (`--fluid` and friends) — `scrollPull.ts` spends its
  reference-px tuning numbers multiplied by this unit. See
  `fluid.config.json` and the CSS package in `assets/styles/`.
- **`ENGAGE_QUERY`** (`lib/constants.ts`) must match `fluid.config.json`'s
  `engageAt` and whatever breakpoint token your CSS calls the same width
  (Tailwind's `lg` in the reference build). Three places, one number.

#!/usr/bin/env node
// verify.mjs — drive a real browser across the desktop matrix (and
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a.startsWith('--')) { console.error(`[verify] unknown flag ${a}`); process.exit(2) }
    else out._.push(a)
  }
  return out
}

const USAGE = `verify.mjs — drive a real browser across the desktop matrix (and mobile sizes)
...
async function main() {
  ...
  const url = args._[0]
  if (!url) {
    console.error('usage: node verify.mjs <url> [--config f] [--out dir] [--widths …] [--heights …] [--mobile W1xH1,W2xH2|none] [--fit-selector sel] [--screens]')
    console.error('       node verify.mjs --help')
    process.exit(2)
  }
  ...
  try {
    ...
    if (!['chromium', 'webkit', 'firefox'].includes(args.browser) || !engine) throw new Error(`[verify] --browser must be chromium, webkit or firefox (got ${JSON.stringify(args.browser)})`)
  } catch (err) {
  ...
  try {
    ...
    console.error(`[verify] could not launch ${args.browser}: ${err.message.split('\n')[0]}\n  Fix: npx playwright install ${args.browser}`)
    process.exit(2)
  }
  const viewports = []
  try {
